...
let reflexiveWords = reflexiveList.trim().split('\n').map(x => {
    ...
    res[1] = res[1].split('-').length > 1 ? res[1].slice(1, -1).split('-') : false;

    return {
        word: res[0],
        irregular: res[1].length ? res[1] : false
    }

    return res;
});

function conjugateReflexive(reflexiveVerbObject, pronoun){
    // get the pronoun prefix (me/te/se) object
    const pronounPrefix = pronounMap[pronoun];

    let reflexiveVerb = reflexiveVerbObject.word.trim();
    let irregular = reflexiveVerbObject.irregular;
...
export {
    pronouns, reflexiveWords, conjugateReflexive
}