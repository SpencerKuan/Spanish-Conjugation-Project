...
class wordGrid {
    constructor(generator, size = 10, cellSize = 600 / size) {
        this.rows = size;
        this.cols = size;
        this.grid = [];
    ...
    draw(){
        ...
        for(let i = 0; i < this.rows; i++){
            for(let j = 0; j < this.cols; j++){
                ...
                fill(255);
                text(this.grid[i * this.cols + j], x + this.cellSize/2, y + this.cellSize/2);
            }
        }
    }
...
function generate(){
    words = reflexive.reflexiveWords.map(x => {
        ...
        const word = reflexive.conjugateReflexive(x, pronoun).replaceAll(" ", "");
    ...
    propogate.then(x => {
        ...
        grid.grid = grid.grid.map(x => x === " " ? pick("abcdefghijklmnopqrstuvwxyzáéíñ".split('')) : x);
    })
}
...
function randomVerb(){
    ...
    return reflexive.conjugateReflexive(word, pronoun);
}
